...
object Printer:
  ...
  def step(s: Step): String = s match
    ...
    case Step.Skolemization(n, f, status, newSymbols, binding, parents) =>
      annotated(n, "plain", f, inference("skolemize", skolemizeInfos(status, newSymbols, binding), parents))
  ...
  /** `skolemize(V, T)` records both the existential variable and its Skolem term,
    * rendered from the step's [[Binding]]. */
  private def skolemizeInfos(
    ...
    List(statusInfo(status), s"new_symbols(skolem, [${newSymbols.mkString(", ")}])")
      ++ binding.map(b => s"skolemize(${b.variable}, ${term(b.term)})")
