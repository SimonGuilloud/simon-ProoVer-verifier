...
class PrinterTest extends munit.FunSuite:
  ...
  test("toString uses the pretty-printer for terms, formulas and steps") {
    ...
    val step: Step = TptpReader.readProof(File(examples, "correct/Proofs/example1_proof.p")).steps.head
  ...
  test("toStringRaw gives the structural rendering, recursing into nested values") {
    ...
    val neg: Step.NegatedConjecture = TptpReader.readProof(File(examples, "correct/Proofs/example1_proof.p")).steps
  ...
  private def allProofFiles: List[File] =
    ...
    walk(examples).filter(f => f.getName.endsWith(".p") && f.getParentFile.getName == "Proofs")
  ...
  test("problem file round-trips: readProblem(print(pf)) preserves steps and header") {
    val pf: ProblemFile = TptpReader.readProblem(File(examples, "correct/Proofs/Problems/example1_c.p"))
    val tmp: Path = Files.createTempFile("proover", ".p")
    try
