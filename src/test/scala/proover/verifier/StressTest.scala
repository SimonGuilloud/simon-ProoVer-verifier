...
class StressTest extends munit.FunSuite:
  ...
  test("COMPLETENESS — free-variable rename in a leaf (should be Verified)") {
    ...
    assertEquals(verdict("comp_freevar_rename"), SzsStatus.VerifiedBad) // BUG: rejected
  }

  test("COMPLETENESS — logically-equivalent goal q&p for p&q (should be Verified)") {
    assertEquals(verdict("comp_commuted_goal"), SzsStatus.VerifiedBad) // BUG: rejected
  }

  test("COMPLETENESS — explicit ![X]p(X) vs free-variable p(X) (should be Verified)") {
    assertEquals(verdict("comp_explicit_forall"), SzsStatus.VerifiedBad) // BUG: rejected
  }
  ...
  test("a name reused at two arities (p/0 vs p/1) verifies — Princess crashed, Vampire is correct") {
    ...
    assertEquals(verdict("robust_arity_crash"), SzsStatus.VerifiedGood)
  }

  test("invalid quantifier-swap step is now refuted (VerifiedBad)") {
    // ?X!Y r does not follow from !Y?X r. Princess returned Unknown (now Timeout);
    // Vampire saturates to CounterSatisfiable, so the verifier correctly rejects it.
    assertEquals(verdict("reject_quantifier_swap"), SzsStatus.VerifiedBad)
  }
