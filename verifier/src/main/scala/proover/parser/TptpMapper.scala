...
object TptpMapper:
    ...
    af.role match
      ...
      case "plain" =>
        ...
        if inf.rule == "skolemize" then
          Step.Skolemization(af.name, form, inf.status, inf.newSymbols, inf.skolemized, inf.binding, inf.parents)
        else
  ...
  private def inference(af: TPTP.FOFAnnotated): Inference =
    ...
    function(src) match
      case Some(("inference", Seq(ruleGt, infoGt, parentsGt))) =>
        ...
        Inference(rule, status, parents, parameters, newSymbolsOf(infoItems), skolemizedOf(infoItems), bindingOf(infoItems))
      case _ =>
  ...
  /** `skolemized(Var)` -> the existential variable being skolemized. */
  private def skolemizedOf(items: Seq[GeneralTerm]): Option[String] =
    items.collectFirst { case Func("skolemized", args) => args.headOption.flatMap(atom) }.flatten

  /** `bind(Var, term)` -> the recorded substitution. */
  private def bindingOf(items: Seq[GeneralTerm]): Option[Binding] =
    items.collectFirst {
      case Func("bind", Seq(v, t)) => atom(v).map(name => Binding(name, generalTerm(t)))
    }.flatten
