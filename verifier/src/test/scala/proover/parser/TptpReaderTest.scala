...
class TptpReaderTest extends munit.FunSuite:
  ...
  test("problem file: leaves with no source") {
    val p: ProblemFile = problem("correct/problem/example1_c.p")
  ...
  test("proof file: header, problem reference, leaf sources") {
    val pf: ProofFile = proof("correct/proof/example1_proof.p")
    assertEquals(pf.problemRef, Some("../problem/example1_c.p"))
    assert(pf.header.text.contains("ProoVer 2026"))

    val axiom: Step.Axiom = pf.steps.collectFirst { case a: Step.Axiom => a }.get
    assertEquals(axiom.source, Some(FileSource("example1_c.p", "a1")))
  ...
  test("proof file: negated_conjecture status fixed to cth in example2") {
    val neg: Step.NegatedConjecture = proof("correct/proof/example2_proof.p").steps
      .collectFirst { case n: Step.NegatedConjecture => n }.get
    assertEquals(neg.status, Status.Cth)
  }

  test("proof file: plain inferences carry rule, status and parents") {
    val plains: List[Step.PlainInference] = proof("correct/proof/example2_proof.p").steps
    ...
    assertEquals(horn.parents, List("a2", "s1", "s2"))
  }

  test("proof file: skolemization fields") {
    val skos: List[Step.Skolemization] = proof("correct/proof/example3_proof.p").steps
    ...
    assertEquals(bride.skolemized, Some("Bride"))
  ...
  test("formula mapping: quantifiers, equality, connectives") {
    ...
    val c: Step.Conjecture = proof("correct/proof/example1_proof.p").steps
    ...
    val neg: Step.NegatedConjecture = proof("incorrect/proof/example2_e_proof.p").steps
      .collectFirst { case n: Step.NegatedConjecture => n }.get
  ...
  test("all example files parse without error") {
    ...
    files.foreach { f =>
      val isProblem: Boolean = f.getParentFile.getName == "problem"
